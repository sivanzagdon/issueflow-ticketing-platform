...
| Logout (invalidate token) | POST /auth/logout | _Requires_: `Authorization: Bearer <accessToken>` | 200 OK          | |
...
| Update a ticket               | PATCH /tickets/:ticketId                   | `{ "version": 1, "title": "...", "description": "...", "status": "IN_PROGRESS", "priority": "MEDIUM", "assigneeId": 3, "dueDate": "2026-04-01T00:00:00Z" }` ( **`version` required** ) | 200 OK / **409 Conflict** | Body on success matches ticket fields (including updated `version`). **409** when `version` is stale. |
